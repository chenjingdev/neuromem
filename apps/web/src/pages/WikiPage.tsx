...
export function WikiPage({ scope }: { scope: Scope }) {
  ...
  return <>
    ...
    {wiki.data && Boolean(wiki.data.sections.length) && <div className={`wiki-layout ${claimId ? "open" : ""}`}><Card className="wiki-document"><header><span className="eyebrow">PROJECT WIKI</span><h2>{wiki.data.title}</h2><small>최근 구성 {formatDate(wiki.data.updated_at)}</small></header>
      <nav aria-label="Wiki 목차">{wiki.data.sections.map(section => <a key={section.id} href={`#wiki-${section.id}`}>{section.heading}</a>)}</nav>
      {wiki.data.sections.map(section => <section id={`wiki-${section.id}`} key={section.id}><h3>{section.heading}</h3><p>{section.body}</p>{Boolean(section.claim_ids?.length) && <div className="citation-chips"><span>근거</span>{section.claim_ids?.map(id => <button key={id} onClick={() => setClaimId(id)}>{id}</button>)}</div>}</section>)}
    </Card>{claimId && <EvidencePanel scope={scope} claimId={claimId} onClose={() => setClaimId(null)} />}</div>}
  </>;
}
