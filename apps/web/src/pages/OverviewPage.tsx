...
export function OverviewPage({ scope, navigate }: { scope: Scope; navigate: (path: string) => void }) {
  ...
  if (loading) return <><PageHeading title="프로젝트 기억" description="현재 프로젝트의 기억을 확인합니다." /><LoadingState /></>;
  if (error || !data) return <><PageHeading title="프로젝트 기억" description="현재 프로젝트의 기억을 확인합니다." /><ErrorState error={error} onRetry={retry} /></>;
  ...
  return <>
    <PageHeading eyebrow={`${data.workspace.name} / ${data.project.name}`} title="기억의 현재 상태" description="현재 Workspace 경계 안에서 General과 선택한 Project 문맥을 사용합니다." actions={<StatusPill state={data.state} />} />
    <div className="metric-grid workspace-product-metrics">
    ...
    <div className="overview-layout">
      <Card className="connection-card">
        ...
        <p>Workspace 관리에서 발급한 MCP credential과 아래 주소를 Codex·Claude에 연결하세요.</p>
      ...
      <Card className="queue-card"><div className="card-heading"><div><span className="eyebrow">SECURITY BOUNDARY</span><h2>Workspace 경계</h2></div><StatusPill state="healthy" /></div><p>브라우저와 에이전트 요청은 Control에서 Workspace·Project 권한을 다시 검증한 뒤 Memory Core로 전달됩니다.</p></Card>
    </div>
    <Card>
