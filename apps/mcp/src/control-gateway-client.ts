...
export class ControlGatewayError extends Error {
  constructor(readonly code: string, readonly httpStatus?: number, readonly retryable = false) {
    super(code);
    this.name = "ControlGatewayError";
  }
}

export interface ControlGatewayRequest {
  body?: JsonObject;
...
export class ControlGatewayClient {
  ...
  constructor(baseUrl: string, timeoutMs = 120_000, maxResponseBytes = 16 * 1_048_576) {
    ...
    catch { throw new Error("Control Gateway URL must be an HTTP(S) URL"); }
    if (!(["http:", "https:"] as string[]).includes(parsed.protocol) || parsed.username || parsed.password) {
      throw new Error("Control Gateway URL must be an HTTP(S) URL without credentials");
    }
  ...
  async request(
    ...
    context: AuthContext,
    bearerToken: string,
    options: ControlGatewayRequest = {}
  ): Promise<unknown> {
    if (!path.startsWith("/api/v1/") || path.startsWith("//")) throw new Error("Control Gateway path must stay under /api/v1");
    if (!bearerToken || /[\r\n\0]/.test(bearerToken)) throw new ControlGatewayError("invalid_credential");
    ...
    try {
      ...
      if (!response.ok) {
        ...
        throw new ControlGatewayError(`gateway_http_${response.status}`, response.status, retryable);
      }
      if (response.status === 204) return null;
      const contentType = response.headers.get("content-type") || "";
      if (!contentType.includes("application/json")) { await response.body?.cancel(); throw new ControlGatewayError("gateway_invalid_json"); }
      return await readBoundedJson(response, this.#maxResponseBytes);
    } catch (error) {
      if (error instanceof ControlGatewayError) throw error;
      if (controller.signal.aborted || (error instanceof Error && error.name === "AbortError")) throw new ControlGatewayError("gateway_timeout", undefined, true);
      throw new ControlGatewayError("gateway_unavailable", undefined, true);
    } finally {
      clearTimeout(timer);
    }
  }
  ...
  async record(
    ...
    if (!this.#ensuredProjects.has(scopeKey)) {
      try {
        ...
        if (!(error instanceof ControlGatewayError) || error.httpStatus !== 403) throw error;
      }
    }
    ...
    try {
      ...
      if (!(error instanceof ControlGatewayError) || error.httpStatus !== 404) throw error;
...
async function readBoundedJson(response: Response, maximum: number): Promise<unknown> {
  if (!response.body) throw new ControlGatewayError("gateway_invalid_json");
  ...
  while (true) {
    ...
    if (size > maximum) { await reader.cancel(); throw new ControlGatewayError("gateway_response_too_large"); }
    chunks.push(value);
  }
  ...
  catch { throw new ControlGatewayError("gateway_invalid_json"); }
}
